...
export default function Galaxy({
  ...
  // Store array/object props in refs so they don't trigger useEffect re-runs
  // when the parent re-renders (e.g. on every keystroke in a form).
  // Inline array literals like focal={[0.5, 0.5]} create a new reference each
  // render, which would otherwise destroy and recreate the WebGL context on each
  // keystroke — causing the white flash/flicker.
  const focalRef = useRef(focal);
  const rotationRef = useRef(rotation);

  // Keep primitive props in a ref too, so the animation loop always sees latest values
  ...
  const propsRef = useRef({
    ...
    rotationSpeed, repulsionStrength, autoCenterRepulsion, mouseInteraction
  });
  useEffect(() => {
    propsRef.current = {
      starSpeed, density, hueShift, disableAnimation, speed,
  ...
  }, [transparent]); // Only restart WebGL if transparency mode changes
